...
while( norm(updateStep) > TOL && iter <= MAX_ITER )
    ...
    % Calculate the Reprojection Error of the line length
    ...
    H = J_p'*J_p...
        + (pointCloud3D.numberOfPoints)/(lineCloud3D.numberOfLines)*(J_l'*J_l)...
...
function J_ll = jacobian_ll(x,y,z,alpha,beta,gamma,lineCloud3D,f)
    J_ll = zeros(lineCloud3D.numberOfLines, 6);
    for j = 1:lineCloud3D.numberOfLines
        z1j = lineCloud3D.linesIn3D(j).startPoint.noisyCoordinatesInWorldFrame(1);
        z2j = lineCloud3D.linesIn3D(j).startPoint.noisyCoordinatesInWorldFrame(2);
        z3j = lineCloud3D.linesIn3D(j).startPoint.noisyCoordinatesInWorldFrame(3);
        ...
        ddeltall_dr1 = [ (2*r1x - 2*r2x)/(2*((r1x - r2x)^2 + (r1y - r2y)^2)^(1/2)), (2*r1y - 2*r2y)/(2*((r1x - r2x)^2 + (r1y - r2y)^2)^(1/2))];
        ddeltall_dr2 = [ -(2*r1x - 2*r2x)/(2*((r1x - r2x)^2 + (r1y - r2y)^2)^(1/2)), -(2*r1y - 2*r2y)/(2*((r1x - r2x)^2 + (r1y - r2y)^2)^(1/2))];
        
        J_lli = ddeltall_dr1*dr1_dphi + ddeltall_dr2*dr2_dphi;
        J_ll(j,:) = J_lli;
    end
return
...
function matrixOfReprojectedLineLengthError = computeLineLengthReprojectionError(matrixOf2DLines, matrixOfReprojectedLines)
    matrixOfReprojectedLineLengthError = zeros(size(matrixOf2DLines,1)/2,1);
    for j = 1:size(matrixOf2DLines,1)/2
        r1x = matrixOfReprojectedLines(2*(j-1)+1,1);
        r1y = matrixOfReprojectedLines(2*(j-1)+2,1);
        r2x = matrixOfReprojectedLines(2*(j-1)+1,2);
        ...
        o2x = matrixOf2DLines(2*(j-1)+1,2);
        o2y = matrixOf2DLines(2*(j-1)+2,2);
        lo = sqrt((o1x - o2x)^2 + (o1y - o2y)^2);
        lr = sqrt((r1x - r2x)^2 + (r1y - r2y)^2);
        matrixOfReprojectedLineLengthError(j) = -(lo - lr);
    end
return
